...
//        System.out.println(excelData);
        ...
        //json格式字符串转成map类型,map类型就是键值对的形式- Jackson,
        ObjectMapper objectMapper = new ObjectMapper();
        Map<String,Object> map = objectMapper.readValue(header, Map.class);
        //header只能传1个请求头,headers方法传入多个请求头，而Map类型多组键值对（多组请求头）
        Response res = given().
                log().all().
                headers(map).
    ...
    public Object[] getDatas(){
        ...
        //设置数据驱动测试 数据源
        //读取Excel中的数据 - easyexcel,head映射到ExcelData实体类的属性值,属性值映射到Excel中的表头
        //实体类：保存从Excel读取的数据,doReadSync()返回的是List集合类型
